...
console.log('admin account-edit.js file is being parsed and executed');

document.addEventListener("DOMContentLoaded", () => {
  console.log('=== admin account-edit.js loaded ===');
  ...
  function handleSave() {
    ...
    // Construct API URL - use AdminCon/update_account
    const apiUrl = updateAccountUrl || (base_url + 'AdminCon/update_account');
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        return response.text().then(text => {
          ...
          if (response.status === 404) {
            throw new Error('404 Not Found: The endpoint AdminCon/update_account was not found. URL tried: ' + apiUrl);
          }
    ...
    .then(data => {
      if (data.success) {
        ...
        if (activeField === 'First_Name' || activeField === 'Last_Name' || activeField === 'Middle_Name') {
    ...
    .catch(error => {
      ...
      showToast("An error occurred while updating the account: " + error.message + ". Please check the browser console (F12) for more details.", 'error');
